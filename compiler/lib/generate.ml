...
module Ctx = struct
  ...
  let initial ~exported_runtime blocks live share debug =
    { blocks; live; share; debug; in_closure = None; continuation_of = None; exported_runtime }
end
...
let fold_children blocks pc f accu =
  ...
  | Return _ | Raise _ | Stop | Resume (_, _, _) | Perform (_, _, _) | Delegate (_, _) | LastApply (_, _, _) -> accu
...
and compile_conditional st queue pc last handler backs frontier interm succs : J.statement_list * Var.Set.t =
  ...
  then
    ...
    | Delegate _ -> Format.eprintf "delegate"
  ...
  let res =
    ...
    | Stop -> flush_all queue ([ J.Return_statement None, loc ], Var.Set.empty)
