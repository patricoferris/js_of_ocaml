...
let get_values k map =
  try Addr.Map.find k map with Not_found -> Addr.Set.empty
...
let build_graph (blocks: block Addr.Map.t) (pc: Addr.t): graph =
  let rec loop (g: graph) (pc: Addr.t) (visited: Addr.Set.t) (anc: Addr.Set.t) =
    if not (Addr.Set.mem pc visited) then begin
      ...
      let preds =
        Addr.Set.fold (fun succ preds -> add_value succ pc preds)
          succs g.preds
        |> Addr.Set.fold (fun back preds -> add_value back pc preds)
          backs
      in
      let loops = Addr.Set.fold Addr.Set.add backs g.loops in
      let handler_succ = match (Addr.Map.find pc blocks).handler with
        | None -> g.handler_succ
        | Some (_, (handler_addr, _)) ->
          Addr.Map.add pc handler_addr g.handler_succ in
      ...
      Addr.Set.fold (fun pc' (g, visited) -> loop g pc' visited anc)
        succs (g, visited)
    end else (g, visited)
  in
  ...
  let (g, _) =
    loop
      ...
      pc Addr.Set.empty Addr.Set.empty in
  g
...
let dominated_by_node (g: graph): Addr.Set.t Addr.Map.t =
  ...
  let all_nodes =
    Addr.Map.fold (fun v _ s -> Addr.Set.add v s)
      g.preds (Addr.Set.singleton g.root) in

  Addr.Set.fold (fun v dominated_by ->
    let not_dominated = explore_avoiding v in
    Addr.Map.fold (fun v' _ dominated_by ->
      if not (Addr.Set.mem v' not_dominated) then
  ...
  let dom_by_node0 =
    ...
    let rec loop node frontier =
      try
        ...
        in
        Addr.Set.fold (fun node' frontier ->
          if Addr.Set.mem node' dom_by_node0 then
            loop node' frontier
          else
            Addr.Set.add node' frontier
        ) succs frontier
      with Not_found ->
        frontier in
...
let trywith_exit_nodes
  ...
  let rec loop node (entry_of_exit, exit_of_entry, visited) =
    let add_entry exit entry entry_of_exit =
      let entries = try Addr.Map.find exit entry_of_exit with Not_found -> Addr.Set.empty in
      ...
      | Pushtrap ((_, _), _, (pc2, _), _) ->
        ...
        let entry_of_exit, exit_of_entry =
          if Addr.Set.is_empty frontier then
            entry_of_exit,
            Addr.Map.add node None exit_of_entry
          else
            let exit = Addr.Set.choose frontier in
            add_entry exit node entry_of_exit,
            Addr.Map.add node (Some exit) exit_of_entry in
  ...
  let entry_of_exit, exit_of_entry, _ =
    loop g.root (Addr.Map.empty, Addr.Map.empty, Addr.Set.empty) in
          ...
          begin match Addr.Map.find pc exit_nodes.exit_of_entry with
            ...
            | Some exit_node ->
              ...
              let entry_defs = List.fold_left (fun m (k, v) ->
                Var.Map.add k v m) Var.Map.empty defsl in
              ...
              let empty_acc =
                ...
                (Var.ISet.empty (),
                 ...
                 Array.make nv (Flow.Phi Var.Set.empty)) in

              Addr.Map.add exit_node (empty_acc, entry_defs) accs_of_open_scopes
          end
        | _ -> accs_of_open_scopes in

      Addr.Set.fold (fun pc' (visited, defs_of_exit_scopes) ->
...
let rec entry_def_of scope_defs entry_defs v =
  ...
  with Not_found ->
    ...
    | Flow.Phi s ->
      let s' = Var.Set.fold (fun v' s' ->
        Var.Set.add (entry_def_of scope_defs entry_defs v') s')
        s Var.Set.empty in
  ...
  let rec loop node (idom: Addr.t Addr.Map.t) =
    ...
    let dom_dom =
      Addr.Set.fold
        (fun node' dom_dom ->
           dom_by node'
           |> Addr.Set.remove node'
           |> Addr.Set.union dom_dom)
        dom Addr.Set.empty
    in
    let idom_node = Addr.Set.diff dom dom_dom in
    let idom = Addr.Set.fold (fun node' idom ->
      Addr.Map.add node' node idom) idom_node idom in
...
let merge_jump_closures jc1 jc2 =
  let m _ a b = 
    ...
    | _ -> assert false in
  { closure_of_jump =
      Addr.Map.merge m jc1.closure_of_jump jc2.closure_of_jump;
...
let cps_blocks st =
  Addr.Map.mapi (cps_block st) st.blocks
...
let f ({ start; blocks; free_pc }: Code.program): Code.program =
  ...
  let st = { new_blocks = Addr.Map.empty, free_pc; blocks; jc; en;
             delimited_by = db;
             defs_of_exit_node = does;
             kx_of_poptrap = Addr.Map.empty } in
